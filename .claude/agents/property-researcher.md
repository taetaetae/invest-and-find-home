...
4. 응답에서 monthly_rent_10k > 0인 항목만 필터링. `min_use_approve_year`가 null이 아니면 `use_approve_date` 연도 >= 값인 항목만, `max_monthly_rent_10k`가 null이 아니면 `monthly_rent_10k <= 값`인 항목만 남긴다.
