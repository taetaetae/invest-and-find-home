...
목표 자산 달성률 하한(`min_achievement_pct`, 기본 70%) 이상 범위에서, 더 비싼 집(달성률 낮음)부터 안전한 선택(달성률 높음)까지 **달성률 구간별로 월세 매물을 (건수 제한 없이) 전부** 추천하는 에이전트 팀 오케스트레이터.
...
→ `region` 수집. `housing_type`은 `"월세"`로 고정한다. 월세 매물을 분석하여 달성률 구간별로 (건수 제한 없이) 전부 추천한다.
...
Agent(
  ...
  prompt: "당신은 부동산 매물 조사 전문가입니다.
    ...
    달성률 정밀 필터·구간 그룹핑(건수 제한 없음)은 strategy-reporter가 담당합니다.
...
Agent(
  ...
  prompt: "당신은 리포트 생성 전문가입니다.
    ...
    6. [핵심] 달성률 < min_achievement_pct(기본 70) 매물은 제외하세요. 하한을 통과한 매물은 건수 제한 없이 전부 표시하세요(상위 N건 컷 없음).
    7. [핵심] 매물을 달성률 구간으로 그룹핑하세요: 🟢안전(≥100%) / 🟡절충(85~100%) / 🔴공격(70~85%). 각 구간을 별도 표(h3 class=band-{level} + report-table)로 렌더링하고(각 구간 건수 제한 없음), 매물명에는 [구명](gu_name)을 muted 텍스트로 병기하세요.
    8. [핵심] 보증금/월세는 별도 컬럼, 보증금 구성(대출/자기자본)도 별도 컬럼으로 분리하세요. 보증금·월세·대출·자기자본·관리비·대출이자·월주거비·투자가능금·예상자산·달성률 등 모든 금액/숫자 컬럼의 <td>에 data-sort-value(만원 단위 정수, 달성률은 % 정수)를 부여해 헤더 클릭 시 숫자 크기순으로 정확히 정렬되게 하세요.
    9. [핵심] 지도 마커는 단지별로 1개이며, 라벨은 단지 내 "월세가 가장 싼" 매물(동률이면 보증금 낮은 쪽)의 보증금/월세를 대표로 "{repDeposit} / 월{minRent}만"으로 표시하세요(COMPLEXES 객체에 minRent·repDeposit 부여).
    10. footer에 "달성률은 투자 자산 기준이며 보증금 회수분 미반영(보수적)" 가정을 명시하세요.
